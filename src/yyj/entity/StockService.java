...
public class StockService {
	...
	// 添加自选股票
	public static List<Stock> addOptional(String stockname) {
		// 获取数据库连接
		Connection conn = UserService.getConnect();
	...
	// 查询股票的数据
	public static List<Stock> queryStock(String key) {
...
//		key = "%" + key + "%";
		...
		String sql1 = "Select * from stock_information where stockname = ?";

		List<Stock> stock = new ArrayList<>();

		try {
			// 查询所有的用户数据，结果放入List<User>
			stock = runner.query(conn, sql1, new BeanListHandler<Stock>(Stock.class), key);
